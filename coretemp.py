...
def probe_temperature():
    """ Returns the CPU and GPU temperature, in degrees centigrade (Celcius) """
...
def plot_temperatures(cpu_temps, gpu_temps):
    ...
    cpu_avg_plot = plt.axhline(y=sum([t for t in cpu_temps.values()])/float(len([t for t in cpu_temps.values()])), xmin=0, xmax=len(cpu_temps), color='r', linestyle='--')
    gpu_avg_plot = plt.axhline(y=sum([t for t in gpu_temps.values()])/float(len([t for t in gpu_temps.values()])), xmin=0, xmax=len(gpu_temps), color='b', linestyle='--')
    cpu_plot = plt.plot(x, cpu_temps.values(), 'ro-')
    gpu_plot = plt.plot(x, gpu_temps.values(), 'bo-')
    locs, labels = plt.xticks(x, cpu_temps.keys())
    plt.ylim(ymax = max([temp for temp in cpu_temps.values() + gpu_temps.values()]) + 1,
             ymin = min([temp for temp in cpu_temps.values() + gpu_temps.values()]) - 1)
    plt.xlim(xmax=len(x), xmin=-1)
    fig.autofmt_xdate() 
    plt.ylabel("Temperature (in Celcius)")
    plt.legend([cpu_plot, gpu_plot], ["CPU", "GPU"])
    plt.suptitle("Evolution of the CPU/GPU temperature", fontsize=25)
    plt.savefig("CPU_GPU_temperature.png")

if __name__ == '__main__': 
    
    cpu, gpu = probe_for_duration(hour=0, minute=30, second=0)
